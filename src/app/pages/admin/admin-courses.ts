        ...
        <section class="rounded-[30px] border border-[var(--color-brand-gold-300)]/26 bg-[linear-gradient(180deg,#fffdf9_0%,#f7efe1_100%)] p-6 shadow-[0_24px_54px_rgba(18,53,36,0.08)]">
          <div class="flex items-start justify-between gap-4">
            <div>
              ...
              <p class="mt-2 text-sm text-[var(--color-brand-green-800)]/68">Admin: contenu, objectifs, programme, PDF, prix et publication.</p>
            </div>
            ...
            <div class="rounded-[24px] border border-[var(--color-brand-gold-300)]/24 bg-white/75 p-4">
              <div class="flex items-center justify-between gap-4">
                ...
                <button type="button" (click)="addObjective()" class="inline-flex items-center gap-2 rounded-full bg-[var(--color-brand-gold-500)] px-4 py-2 text-sm font-semibold text-[var(--color-brand-green-900)]">
                  <mat-icon class="!h-[18px] !w-[18px] !text-[18px]">add</mat-icon> Ajouter objectif
                </button>
              </div>
            ...
            <div class="rounded-[24px] border border-[var(--color-brand-gold-300)]/24 bg-white/75 p-4">
              <div class="flex items-center justify-between gap-4">
                ...
                <button type="button" (click)="addContentItem()" class="inline-flex items-center gap-2 rounded-full bg-[var(--color-brand-gold-500)] px-4 py-2 text-sm font-semibold text-[var(--color-brand-green-900)]">
                  <mat-icon class="!h-[18px] !w-[18px] !text-[18px]">add</mat-icon> Ajouter contenu
                </button>
              </div>
            ...
            <div class="rounded-[24px] border border-[var(--color-brand-gold-300)]/24 bg-white/75 p-4">
              <div class="flex items-center justify-between gap-4">
                ...
                <button type="button" (click)="addChapter()" class="inline-flex items-center gap-2 rounded-full bg-[var(--color-brand-gold-500)] px-4 py-2 text-sm font-semibold text-[var(--color-brand-green-900)]">
                  <mat-icon class="!h-[18px] !w-[18px] !text-[18px]">add</mat-icon> Ajouter chapitre
                </button>
              </div>
            ...
            <div class="rounded-[24px] border border-[var(--color-brand-gold-300)]/24 bg-white/75 p-4">
              <div class="flex items-center justify-between gap-4">
                ...
                <button type="button" (click)="addModule()" class="inline-flex items-center gap-2 rounded-full bg-[var(--color-brand-gold-500)] px-4 py-2 text-sm font-semibold text-[var(--color-brand-green-900)]">
                  <mat-icon class="!h-[18px] !w-[18px] !text-[18px]">add</mat-icon> Ajouter module
                </button>
              </div>
              <div formArrayName="moduleItems" class="mt-4 space-y-4">
                @for (module of moduleItems.controls; track $index) {
                  <div [formGroupName]="$index" class="rounded-[22px] p-4 bg-[var(--color-brand-cream)]/75">
                    ...
                    <div class="mt-3 flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
                      <div class="text-sm text-[var(--color-brand-green-800)]/70">{{ module.value.pdfName || 'Aucun PDF selectionne' }}</div>
...
export class AdminCoursesComponent implements OnInit {
  ...
  private createModuleGroup(module?: CourseModuleItem) {
    return this.fb.nonNullable.group({ id: [module?.id ?? `module-${Date.now()}-${this.moduleItems.length}`], title: [module?.title ?? ''], pdfName: [module?.pdfName ?? ''], pdfDataUrl: [module?.pdfDataUrl ?? ''] });
  }
  private createContentItemGroup(item?: CourseContentItem) {
    return this.fb.nonNullable.group({ id: [item?.id ?? `content-${Date.now()}-${this.contentItems.length}`], text: [item?.text ?? ''] });
  }
  ...
  addContentItem(): void { this.contentItems.push(this.createContentItemGroup()); }
  ...
  onModulePdfSelected(event: Event, index: number): void {
    ...
    if (!file) return;
    const reader = new FileReader();
    reader.onload = () => this.moduleItems.at(index).patchValue({ pdfName: file.name, pdfDataUrl: String(reader.result ?? '') });
    reader.readAsDataURL(file);
  }
  ...
  save(): void {
    ...
    request.subscribe({
      next: () => { this.resetForm(); this.load(); this.feedback.set({ type: 'success', text: 'La formation a bien ete enregistree.' }); setTimeout(() => this.feedback.set(null), 5000); },
      error: (err: any) => { this.feedback.set({ type: 'error', text: err?.error?.error || err?.message || 'La creation a echoue.' }); }
    });
  }

  remove(courseId: string): void {
    this.staff.deleteAdminCourse(courseId).subscribe(() => this.load());
  }
}
