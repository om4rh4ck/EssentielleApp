...
@Component({
  ...
  template: `
    <app-dashboard-layout title="Paramètres" [menuItems]="menuItems">
      <div class="grid gap-6 xl:grid-cols-[0.9fr_1.3fr]">
        ...
        <section class="rounded-[28px] bg-white p-6 shadow-[0_20px_45px_rgba(0,0,0,0.04)]">
          <h2 class="font-serif text-3xl text-[var(--color-brand-green-900)]">Modifier le profil admin</h2>
...
export class AdminSettingsComponent implements OnInit {
  ...
  ngOnInit(): void {
    this.staff.getAdminProfile().subscribe((data) => {
  ...
  save(): void {
    ...
    this.staff.updateAdminProfile(this.form.getRawValue()).subscribe((data) => {
      this.profile.set(data);
      this.initial.set(data.name.charAt(0).toUpperCase());
